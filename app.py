...
# ==============================
# Sidebar (FORA de qualquer if/else)
# ==============================
...
# ==============================
# Página: Rafinha (CORRIGIDA E SEGURA)
# ==============================
elif pagina == "Rafinha":
    ...
    # CSS para caixa fixa no topo central
    st.markdown(
        """
        <style>
    ...
    # Exibição segura das mensagens (SEM KeyError)
    st.markdown('<div class="fixed-chat-container">', unsafe_allow_html=True)
    ...
    if historico:
        for msg in historico[-20:]:
            # ✅ SEMPRE usa .get() para evitar KeyError
            ...
            if resposta_msg:
                ...
                st.markdown(
                    f'<div class="rafinha-message">Rafinha: {resposta_msg}</div>',
                    unsafe_allow_html=True
                )
    else:
        st.markdown(
